...
SECTION_BLURB = {
    ...
    "examples": "Demo-seed content illustrating the work-item model (not engine logic).",
    "archive/build-log": "PMO v1 build process log — provenance, not prompts.",
}
...
def classify(mem: dict) -> tuple[str, str]:
    ...
    if tags & EXAMPLE_TAGS:
        return "examples", kebab(title_of(mem["content"]))
    return "archive/build-log", kebab(title_of(mem["content"]))
